...
public class TP_Motor : MonoBehaviour {
	...
	//Only gets called when TP_Controller defines MoveVector
	...
	void ProcessMotion(){
		//Transform MoveVector to worldspace relative to roatation
		MoveVector = transform.TransformDirection (MoveVector);
		//Normalize vector if magnitude is greater than 1
		if(MoveVector.magnitude > 1.0f) MoveVector.Normalize ();
		//Apply Sliding
		ApplySlide ();
	...
	void ApplySlide(){
		if(!TP_Controller.CharacterController.isGrounded) return;
